...
for (let i=0; i<currentInfection.length; i++) {
    if (currentInfection[i] === 1) { //Recorremos el array buscando donde hay infectados = 1
        if (i>0) { //asegura que no esta en i=0 porque ai lo estuviera añadiria un 1 en la posicion -1
            nextDay[i - 1] = 1; // si i mayor que 0 se contagia al anterior
        }

        if (i !== currentInfection.length-1) { //asegura que i no esta en la ultima posicion del arrauy porque ai lo estuviera añadiria un 1 en la posicion currentInfection.length+1
...
function daysToPopulationFullInfected(initialSituation) {
    let numDays = 0;
        for (let i=0; i<initialInfection.length; i++) { //recorremos el array de la situación inicial
            if (initialInfection.indexOf(1) === -1) {
                console.log("La población nunca se infectará"); //si no existen 1 es que no hay ningun infectado ==> la población nunca se infectará, 
                    ...
                    while (initialInfection.indexOf(0) !== -1) { //la condicion es que el while se ejecute mientras existan 0 (0!==-1)
                        initialInfection = infectEvolution(initialInfection); //aplicamos la función sobre como va a evolucionar la infección, La infeccion inicial va variando al aplicar la funcion sobre ella
...
let daysToFullInfection = daysToPopulationFullInfected(initialInfection);

console.log(`Toda la población se infecta en ${daysToFullInfection} días, es decir, en ${daysToFullInfection*(24)}h`);
